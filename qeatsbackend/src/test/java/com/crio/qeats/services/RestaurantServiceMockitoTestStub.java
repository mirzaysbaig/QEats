...
//@MockitoSettings(strictness = Strictness.STRICT_STUBS) // not neede when we use extendwith it automaticaaly cintializes mock 
@DirtiesContext
@ActiveProfiles("test")
...
What it does: Initializes all the @Mock annotated fields in the test class.

Why it's needed: Without this, the @Mock fields (restaurantRepositoryServiceMock) remain null, and any attempt to use them would result in a NullPointerException.

Note: This is actually an older way of initializing mocks. Since JUnit 5, it's recommended to use @ExtendWith(MockitoExtension.class) (which your class already has), so this line is redundant.
   */

  @BeforeEach // setting mock 
  void setup() {
  ...
  public void initializeRestaurantObjects() throws IOException {
    ...
    // now initalizing the restaurant 
 ...
 //lenient() makes Mockito ignore unnecessary stubbing warnings.
